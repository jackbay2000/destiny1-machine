...
import linux_sys  # Linux platform backend (replaces ctypes.windll.user32)
...
class InputBridge:
    def __init__(self, cfg: dict):
        ...
        # Cached focus result updated by _poll_focus() at 10 Hz — avoids
        # querying the window title every tick at 250 Hz.
        self._chiaki_title_ok = False

        self._lock         = threading.Lock()
        self._action_count: dict[str, int] = defaultdict(int)
        self._held_keys:    set[str]        = set()

        self._cx, self._cy = linux_sys.get_screen_center()
    ...
    def _update(self):
        ...
        # Right stick — poll cursor position, reset to centre each tick
        if self.captured:
            x, y = linux_sys.get_cursor_pos()
            dx = float(x - self._cx)
            dy = float(y - self._cy)
            linux_sys.set_cursor_pos(self._cx, self._cy)
            ads  = ac.get("l2", 0) > 0
    ...
    def _poll_focus(self):
        """Background thread: updates _chiaki_title_ok at 10 Hz so _update()
        can read a cached value instead of hitting Xlib every tick.
        Skips the update when X11 is unavailable so the True fallback set
        in start() is not overwritten by an empty title string."""
        while self.running:
            if linux_sys.x11_available():
                title = linux_sys.get_focused_window_title()
                self._chiaki_title_ok = "chiaki" in title.lower()
            time.sleep(0.1)

    def _is_chiaki_focused(self) -> bool:
        return self._chiaki_title_ok
    ...
    def _toggle_capture(self):
        ...
        linux_sys.show_cursor(not self.captured)
        if self.captured:
            center = linux_sys.get_focused_window_center()
            if center:
    ...
    def start(self):
        ...
        if not linux_sys.x11_available():
            print("[WARN] X11 unavailable — cursor hiding and focus detection disabled.")
            print("       Input will be forwarded at all times once mouse is captured.")
            # Make focus checks always return True so input isn't silently dropped
            self._chiaki_title_ok = True

        print("=" * 50)
        print("  destiny1-mk  |  M&K → DS4 virtual controller")
        print("=" * 50)
        ...
        focus_thread = threading.Thread(target=self._poll_focus, daemon=True)
        focus_thread.start()
